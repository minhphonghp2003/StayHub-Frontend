...
function MenuList({ selectedTier }: { selectedTier?: Tier | null }) {
    ...
    let assignMenu = async () => {
        const result = await toastPromise(
            tierService.assignMenusToTier([...assignedMenus], selectedTier?.id ?? 0),
            {
    ...
    let fetchAssignedMenus = async (signal: any) => {
        ...
        let result = await tierService.getMenusOfTier(selectedTier.id ?? 0, signal)
        if (result) {
            setAssignedMenus(new Set(result))
            setInitAssignedMenus(new Set(result))
        }
    ...
    return (
        <div className=''>
            ...
            <ConfirmDialog isOpen={isOpenConfirm} closeModal={setOpenConfirm} title={`Phân quyền cho Tier`} desc={`Xác nhận gán ${assignedMenus.size} menu cho Tier ${selectedTier?.name}`} action={
